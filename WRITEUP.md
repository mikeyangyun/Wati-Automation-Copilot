...
TypeScript pnpm monorepo: **`shared`** (one Zod schema, types, fixtures) · **`server`** (Fastify, executor FSM, agents, validator, simulator) · **`web`** (Vite + React Flow + Zustand). **7 REST endpoints**, in-memory store, **578 tests** (73 / 249 / 256). CI gate covers typecheck / lint / test / build + an in-process simulation smoke harness.
...
- **Test design.** 578 tests are mine in shape — Cursor drafted cases, I tightened invariants, added the structural-vs-semantic interaction tests, and the in-process simulation smoke harness (`pnpm --filter server simulation-smoke`).
